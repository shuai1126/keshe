...
void SearchResult::on_pushButton_2_clicked()
{
    ...
    if(ui->tableWidget->item(ui->tableWidget->currentItem()->row(), 5)->text() == "0" && ui->tableWidget->item(ui->tableWidget->currentItem()->row(), 8)->text() == "0" && ui->tableWidget->item(ui->tableWidget->currentItem()->row(), 11)->text() == "0"){
        QMessageBox::critical(this,"错误",tr("已无余票！"));
        return;
    }
    if(Type == "U"){
    ...
    if(Type == "S"){
        p->sendDataTrain(TrainNumber,Data3);
        p->show();//跳转到售票员购票界面
        this->close();
    }
}
